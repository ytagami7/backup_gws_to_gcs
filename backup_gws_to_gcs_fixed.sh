...
################################################################################
# GWS to GCS Backup Script (Base + Incremental + Shared Drives) - NO LOOP
################################################################################
#
# --- 使用方法 ---
...
backup_user_mydrive() {
  ...
  if is_first_backup_mydrive "$user" "$safe_user"; then
    ...
    log "Executing: rclone copy (無限ループ対策: --skip-links --max-depth 100)"
...
log "=========================================="
log "GWS to GCS Backup Started (MyDrive + Shared Drives)"
log "Date: $(date)"
log "Mode: $MODE_INFO"
log "無限ループ対策: --skip-links --max-depth 100"
log "=========================================="
...
log ""
log "=========================================="
log "GWS to GCS Backup Completed (MyDrive + Shared Drives)"
log "Date: $(date)"
log "Mode: $MODE_INFO"
log "=========================================="
